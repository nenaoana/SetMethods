...
robust.intersections <- function(results1, results2, sol1 = 1, sol2=1, use.tilde = TRUE)
{
  ...
  thintersect$S1S2 <- sop(intersectExp(emp1,emp2))
  thintersect$s1S2 <- sop(intersectExp(negateExp(emp1),emp2))
  thintersect$S1s2 <- sop(intersectExp(emp1,negateExp(emp2)))
  thintersect$s1s2 <- sop(intersectExp(negateExp(emp1),negateExp(emp2)))
